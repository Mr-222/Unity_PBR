#ifndef CUSTOM_CLOTH_PASS_INCLUDED
#define CUSTOM_CLOTH_PASS_INCLUDED
...
Varyings ClothPassVertex(Attributes input)
{
    Varyings output;
...
float4 ClothPassFragment(Varyings input) : SV_TARGET
{
    UNITY_SETUP_INSTANCE_ID(input);
    ...
    surface.sheenColor = GetSheenColor(input.baseUV).rgb;
    surface.subsurfaceColor = GetSubsurfaceColor(input.baseUV).rgb;
    ...
    CustomBRDFData brdf = InitializeBRDFData(surface);

    float3 luminance = GetLighting(LIGHTMAP_UV_FRAGMENT_DATA, surface, brdf, 2);

    luminance += surface.emission;

    // Cloth should not have blooming effect
    return float4(saturate(luminance), 1.0);
}
